...
def worker_thread():
    ...
    try:
        ...
        if not job.is_stopped():
            # ---- Send to Rentry ----
            job.add_log("info", "Generating results for Rentry...")
            lines = []
            ...
            if lines:
                ...
                text_to_publish = "no email valid"
            
            try:
                payload = {"text": text_to_publish}
                if job.rentry_custom_id:
                    payload["url"] = job.rentry_custom_id
                
                resp = httpx.post("https://rentry.co/api/new", data=payload, timeout=10)
                resp_text = resp.text
                
                url_match = re.search(r'"url"\s*:\s*"([^"]+)"', resp_text)
                edit_match = re.search(r'"edit_code"\s*:\s*"([^"]+)"', resp_text)
                
                if url_match:
                    paste_url = url_match.group(1).replace('\\/', '/')
                    edit_code = edit_match.group(1) if edit_match else None
                    job.add_log("success", f"Rentry Paste URL: {paste_url}")
                    if edit_code:
                        job.add_log("info", f"Rentry Edit Code: {edit_code}")
                else:
                    job.add_log("error", f"Failed to create Rentry paste: {resp_text}")
            except Exception as ex:
                job.add_log("error", f"Rentry upload failed: {ex}")
            # -------------------------
